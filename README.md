...
`main` 브랜치에 병합되어 배포된 API 문서는 [docs.gorani.me/LaunchingService-Android](https://docs.gorani.me/LaunchingService-Android/)에서 확인할 수 있습니다.
...
Dokka HTML은 `launching-service/build/dokka/html`에 생성되며 저장소에 커밋하지 않습니다. `Deploy Dokka` 워크플로는 `main` 변경 시 해당 출력을 `swift-man/docs`의 `LaunchingService-Android/` 경로에 배포합니다. 이 저장소에는 대상 문서 저장소의 쓰기 가능한 배포 키와 대응하는 `DOCS_DEPLOY_KEY` secret이 필요합니다.
